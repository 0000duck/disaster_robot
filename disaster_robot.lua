function sysCall_init()
    ...
    peopleSensor = sim.getObjectHandle("bubbleRob_sensingNose1")
    bill = sim.getObjectHandle("Bill#0")
...
function sysCall_actuation() 
    ...
    person=sim.readProximitySensor(peopleSensor)
    -- If we detected something, we set the backward mode:
    if (result>0) then backUntilTime=sim.getSimulationTime()+4 end 
    if (person > 0) then
        print("found")
        sim.setObjectPosition(bill, tree, {0.1,0,0})
    end
